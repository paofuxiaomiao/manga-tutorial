...
const IMAGES = {
  canvas: asset("libtv/canvas-nodes.png"),
  slash: asset("libtv/slash-tools.png"),
  imageTools: asset("libtv/image-tools.png"),
  videoModel: asset("libtv/video-model.png"),
};
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-slate-100">
      ...
      <footer className="border-t border-white/5 py-8 text-center text-xs text-slate-500">
        LibTV 交互课程 · 内容来源于飞书使用指南 · 课程化改编用于学习和查阅
      </footer>
    </div>
  );
}
      ...
      <div className="relative mx-auto grid min-h-[520px] max-w-6xl items-center gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[1fr_0.9fr] lg:px-8">
        ...
        >
          <div className="overflow-hidden rounded-xl border border-white/10 bg-[#111827] shadow-2xl shadow-cyan-950/20">
            ...
            <div className="p-4">
              <img src={IMAGES.canvas} alt="LibTV 无限画布节点示例" className="aspect-video w-full rounded-lg object-cover" />
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          <p className="mb-5 text-sm leading-7 text-slate-400">
            无限画布是 LibTV 的主战场。进入首页后点击「开始创作」即可新建画布；画布内双击空白处可新建节点，也可以把图片、视频、音频直接拖进画布。
          </p>
          <div className="overflow-hidden rounded-xl border border-white/8 bg-white/[0.03]">
            <img src={IMAGES.canvas} alt="无限画布节点" className="aspect-video w-full object-cover" />
          </div>
        </div>
        <div className="space-y-3">
          {[
            ["项目菜单栏", "管理项目、返回主页、进入全部项目、创建或删除项目。"],
            ["画布左侧栏", "添加节点、打开工作流、查看资产、历史记录和教程。"],
            ["个人中心", "分享/发布、消息中心、会员中心、积分消耗和账号信息。"],
            ["小地图导航", "在复杂画布中查看全局节点分布，拖动高亮框快速定位。"],
          ].map(([title, desc]) => (
